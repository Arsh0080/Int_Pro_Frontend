      ...
      <div className="card">
        ...
        {isEditing ? (
          <div className="form-grid">
            ...
            {/* Skills */}
            <h3>Skills</h3>
            {["languages", "frontend", "backend", "database"].map((cat, i) => (
              <div className="skill-section" key={`${cat}-${i}`}>
                <strong>{cat.toUpperCase()}</strong>
                {(formData.skills[cat] || []).map((s, j) => (
                  <div className="input-row" key={`${cat}-${j}`}>
                    <input
                      value={s}
                      onChange={(e) =>
                        handleSkillChange(cat, j, e.target.value)
                      }
                    />
                    <button
                      className="btn delete"
                      onClick={() => removeSkillItem(cat, j)}
                    >
                      X
                    </button>
                  </div>
